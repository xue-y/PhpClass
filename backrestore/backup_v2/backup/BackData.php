...
class BackData{
	...
	public function back_exec($host='127.0.0.1',$db,$dbuser='',$dbpw='',$table=array(),$charset='utf8',$prot=3306)
	{
		...
		// sql_insert 是父类下的方法
	//	$this->backup->sql_insert(); // 执行写入操作【备份】
		$this->sql_insert();
	}
    ...
    {
        $row=$this->backup->table_structure($table_name); // 调用的是子类的 查询表结构语句---------------------子类方法
    ...
    public function sql_insert()
    {
      //  echo "开始备份";
        ...
        $table_name=$this->backup->table_name(); //---------------------------------------------调用子类方法
       ...
       // $back_file=$this->back_dir.$this->db.'_'.date('Ymd',time()).$this->back_file_fu;
        ...
        foreach($table_name as $k=>$v) // 循环所有的表
        {
            ...
            $lock_re=$this->backup->lock_table($v); // 锁表  --------------------------------------子类方法

            $table_field_type=$this->backup->field_type($v); // 字段类型 --------------------------------------子类方法

            $sql_table=$this->sql_table($v);    // 表结构sql 语句

            $strlen=strlen($sql_table); // 记录sql 长度

            $insert=$this->backup->select_insert($v); // 表数据  --------------------------------------子类方法
            ...
            $this->backup->unlock_table($lock_re);  // 解表  --------------------------------------子类方法
...
$back->back_exec('127.0.0.1','medf','root','123',$table_name);*/
