...
#### backup_v1 版本 
	|---> 缺点：子类与父类之间有互相调用有耦合<br/>
	|---> 优点：子类与父类中的函数属性都为 protected 类型<br/>
	|---> 使用函数调用<br/>

#### backup_v2 版本
	|---> 缺点：子类与父类中的函数属性调用调的地方都为 public 类型<br/>
	|---> 优点：父类没有调用子类方法<br/>
	|---> 使用类调用 <br/>
...
4：使用备份类备份的数据，创建表语句之前都有一个删除同表名的sql语句，
使用phpmyAdmin（可能使用其他工具导出查看是否数据格式与 备份类数据格式一致，非常重要！！！）导出的sql
表创建语句前没有删除同名表语句，更改原表名为 【临时表名】不可 注释省略
</pre>
